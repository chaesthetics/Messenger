...
const Navbar = () => {
  ...
  return (
    <>
      <nav className="flex justify-between items-center px-8 md:px-16 py-7 md:py-9 border-b-[1px] sticky top-0 bg-white bg-opacity-80 z-50">
        <div className="flex items-center space-x-2">
          {/* Logo SVGs */}
          <svg aria-label="Vercel logomark" height="22" role="img" viewBox="0 0 74 64"><path d="M37.5896 0.25L74.5396 64.25H0.639648L37.5896 0.25Z" fill="var(--geist-foreground)"></path></svg>
        ...
        {/* Desktop Menu */}
        <div className="space-x-5 flex items-center flex-row hidden md:flex">
          {['Features', 'Desktop App', 'Privacy & Safety', 'For Developers'].map((label, i) => (
            <div className="group" key={i}>
              <Link href={`/${label.replace(/ &|\s+/g, '').toLowerCase()}`} className="text-[15px] text-black font-semibold">
                {label}
              </Link>
              <div className="w-full h-[3px] bg-blue-700 rounded-md opacity-0 group-hover:opacity-100"></div>
            </div>
          ))}
        </div>
        ...
        <button className="block md:hidden" onClick={() => setIsOpen(!isOpen)}>
          <Bars3Icon className="h-6 w-6 text-gray-800 transition-transform" />
        </button>
      </nav>

      {/* Mobile Drawer */}
      <div className={`fixed top-0 right-0 h-full w-64 bg-white z-50 shadow-lg transform transition-transform duration-300 ${isOpen ? 'translate-x-0' : 'translate-x-full'} md:hidden`}>
        <div className="p-6 pt-8 pr-8 flex flex-col space-y-5">
          <button onClick={() => setIsOpen(false)} className="self-end pb-4">
            <XMarkIcon className="h-6 w-6 text-gray-800" />
          </button>
          {['Feature', 'Desktop App', 'Privacy & Safety', 'For Developers'].map((label, i) => (
            <Link
              key={i}
